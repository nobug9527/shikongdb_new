...
namespace Sk_B2BAPI.Admin.YwtSalesOrder.ashx
{
    /// <summary>
    /// ReturnJson1 的摘要说明
    /// </summary>
    public class ReturnJson : IHttpHandler, IRequiresSessionState
    {
        ...
        /// <param name="proc"></param>
        /// <returns></returns>
        protected string ReturnListJson(string json, string proc, string userId, string entid)
        {
            string rJson = "";
            SqlParameter[] param = (JsonMethod.ListParameter(json, userId, entid)).ToArray();//动态解析json参数
            SqlRun sql = new SqlRun(SqlRun.sqlstr);
            ...
            if (ds.Tables.Count > 0)
            {
                rJson = JsonMethod.DataColumnToJson(ds);
            }
            else
            {
                string error = "无数据";
                rJson = JsonMethod.GetError(1, error);
            }
            return rJson;
        }
        /// <summary>
        ...
        /// <param name=""></param>
        /// <returns></returns>
        protected string ReturnAddCartMx(string json, string proc, string userId, string entid)
        {
            return "";
        }
