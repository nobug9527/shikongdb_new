...
//获取商品分类列表
function QueryOneCategoryList(obj) {
        ...
        success: function (result) {
            ...
            if (type == '0') {
                ...
                for (var i = 0; i < obj.length; i++) {
                    ...
                    if (obj[i]["class_layer"] == "1") {
                        html += '<a style="text-decoration:none;margin-right:5px;" class="btn btn-primary radius"  href="goods_Son_category_list.html?id=' + obj[i]["id"] + '&name=' + obj[i]["title"]+'" title="查看子类">查看子类</a> ';
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (type == '0') {
                ...
                $("#txtContent").val();
                $("#txtCategorysImg").val(obj["img_url"]);//封面图片
                layer.close(index);
            }
            else {
                layer.close(index);
...
function Save_Category() {
    ...
    var img_url = $("#txtCategorysImg").val();
    var id = GetQueryString("id");
    if (id == "") {
        id = "";
    }
...
function tz(id) {
    window.location = "goods_Son_category_list.html?id=" + id;
}